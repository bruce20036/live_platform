...
if __name__ == '__main__':
    ...
    try:
        for i in range(BOX_AMOUNT):
            ...
            get_media_process = Process(name="Media_Box %d"%(i+1),
                                        target=run_zmq_MEDIA_BOX,
                                        args=(box_id, IP, PORT, rdb))
            publish_process = Process(name="Publish_Box %d"%(i+1),
                                      target=run_zmq_PUB_BOX,
                                      args=(box_id, IP, PORT, rdb))
            
            get_media_process.start()
            publish_process.start()
            box_process.append([box_id, get_media_process, publish_process])
        ...
        while True:
            for i in range(BOX_AMOUNT):
                if not box_process[i][1].is_alive():
                    get_media_process = Process(name="Media_Box %d"%(i+1),
                                                target=run_zmq_MEDIA_BOX,
                                                args=(box_id, IP, PORT, rdb))
                    get_media_process.start()
                    box_process[i][1] = get_media_process
            time.sleep(3)
    except KeyboardInterrupt:
        for i in range(BOX_AMOUNT):
            box_process[i][1].terminate()
            box_process[i][2].terminate()
            box_process[i][1].join()
            box_process[i][2].join()
